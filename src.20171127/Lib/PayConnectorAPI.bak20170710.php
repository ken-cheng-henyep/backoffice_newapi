...
class PayConnectorAPI {
	...
	function __construct($debug=false) {
		...
		$this->database_conn = new \mysqli($db_host, self::DATABASE_USER, self::DATABASE_PASSWORD, self::DATABASE_NAME);
		...
		// Added at 2017-06-22 17:08 
		// Read the address for server assigned enviornment. if nothing there, use default value.
        \DB::$host = $db_host;
...
/*
    ...
    public function getDatabaseTransactions($start, $end, $mid='', $tid='', $limit=0, $offset=0) {
		...
		tx.product, 
		tx.ip_address, 
		coalesce(ght.BANK_NAME, g.BANK_NAME) as bank, 
		coalesce(ght.BANK_CODE, g.BANK_CODE) as bank_code, 
		...
		tx.internal_id, coalesce(ght.merchant_no, g.merchant_no) as processor_account_no, 
		coalesce(ght.fee, g.fee) as processor_fee, 
		wecollect_fee_usd as wecollect_fee , 
		coalesce(wecollect_fee_cny, GREATEST(m.settle_min_fee_cny, tx.AMOUNT * m.settle_fee / 100 )) AS wecollect_fee_cny , 
		(case when ght.id_number is not null then ght.id_number else tx.id_card_number end) as id_card_number, 
		(case when ght.card_number is not null then ght.card_number else tx.card_number end) as card_number, 
		(case when ght.phone_no is not null then ght.phone_no else tx.mobile_number end) as mobile_number, 
		tx.verified_name, 
		tx.site_id, 
		tx.site_name,

		(case when ght.payment_time is not null then ght.payment_time when g.transaction_time is not null then g.transaction_time else null end) as processor_state_time 
		FROM %s m, %s tx 
