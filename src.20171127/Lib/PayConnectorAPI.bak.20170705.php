...
*/
    ...
    public function saveToExcel2($data, $filename, $ext = '.xlsx') {
        ...
        if (!isset($data['sheets']) || !is_array($data['sheets'])) {
            $data['sheets']['Sheet1'] = $data;
        }

        $i = 0;
        foreach ($data['sheets'] as $name=>$sdata) {
            $meta = array_keys(array_change_key_case($sdata[0]));
            PHPExcel_Cell::setValueBinder(new PHPExcel_Cell_WcColumnValueBinder($meta));

            $sheet = $excel->createSheet($i);
            $sheet->setTitle("$name");
            $sheet->fromArray($meta, null, 'A1');
            $sheet->fromArray($sdata, null, 'A2');
            //column auto width
            $lastCol = \PHPExcel_Cell::stringFromColumnIndex(count($meta) - 1);
            foreach (range('A', $lastCol) as $colidx)
