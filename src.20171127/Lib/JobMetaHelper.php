...
class JobMetaHelper
{
     ...
     * @param      string|array  $requestedStatus  The requested status
     *
     ...
     *
     * @param      string   $job_id  The job identifier
     * @param      string   $failure_message  The failure message
     *
     * @return     boolean  Return true if record saved successfully
     */
    public static function markFailure($job_id, $failure_message = '')
    {
        $entity = self::getEntity($job_id);
        if (empty($entity)) {
            return false;
        ...
        if (self::updateEntity($job_id, ['status'=>self::STATUS_FAIL, 'failure_date'=>date('Y-m-d H:i:s'), 'failure_message'=>$failure_message])) {
            return true;
        }
        return false;
    }
     ...
     * @return     boolean  Return true if record saved successfully
     */
    public static function markCancelled($job_id, $failure_message = '')
    {
        $entity = self::getEntity($job_id);
        if (empty($entity)) {
            return false;
        ...
        if (self::updateEntity($job_id, ['status'=>self::STATUS_CANCELLED, 'failure_date'=>date('Y-m-d H:i:s'), 'failure_message'=>$failure_message])) {
            return true;
        }
        return false;
    }
     ...
     * Gets the table.
     *
     * @return     <type>  The CakePHP table object .
     */
