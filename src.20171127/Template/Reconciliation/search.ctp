...
$(function() {
    ...
    var searchInfo = {start_date: null, end_date: null, txids:[], reconciliation_batch_id: null};
...
<?php endif;?>
    ...
    var reconciliationGridDs = new kendo.data.DataSource({
        ...
        transport: {
            read: {
                // the remote service url
                url: '<?=$this->Url->build([ "action" => "searchBatch" ])?>',
    ...
    $('.users.form .download-btn').on('click', function(){
        $.ajax({
            ...
            data: {txid: searchInfo.txids.join(',')},
            type:'post',
            dataType:'json',
    ...
    var $activityIndicator = $('<div class="k-loading-mask" style="width:100%;height:100%; position:absolute; top:0; left:0;"><span class="k-loading-text">Loading...</span><div class="k-loading-image"><div class="k-loading-color"></div></div></div>');
