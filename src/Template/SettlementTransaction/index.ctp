...
<?php endif;?>
    ...
    columns.push({
        ...
        template: function(entity){
            if(entity.settlement_status == 'UNSETTLED') {
                return $('<a href="javascript:void(0);" class="btn-tx-hold">Hold</a>')
                .attr('data-id', entity.id)
                .get(0).outerHTML
            }
            if(entity.settlement_status == 'WITHHELD') {
                return $('<a href="javascript:void(0);" class="btn-tx-release">Release</a>')
                .attr('data-id', entity.id)
                .get(0).outerHTML
            }
            return '';
    ...
    var txRelease = function(txid) {
        showIndicator()
        // Getting last settlement date by querying holiday
        $.post('<?=$this->Url->build([ "controller"=>"SettlementTransaction", "action" => "txRelease" ])?>', {txid: txid}, function(rst){
            hideIndicator();
            if (rst.status == 'done') {
                dataSource.fetch();
            }else{
    ...
    var txHold = function(txid) {
        showIndicator()
        // Getting last settlement date by querying holiday
        $.post('<?=$this->Url->build([ "controller"=>"SettlementTransaction", "action" => "txHold" ])?>', {txid: txid}, function(rst){
            hideIndicator();
            if (rst.status == 'done') {
                dataSource.fetch();
            }else{
    ...
    $('body').on('click', '.btn-tx-hold', function(evt){
        evt.preventDefault();
        var data = $(this).data();
        if(data.id) {
            txHold(data.id);
        }
    }).on('click', '.btn-tx-release', function(evt){
        evt.preventDefault();
        var data = $(this).data();
        if(data.id) {
            txRelease(data.id);
