...
*/
    ...
    public function validateInstantReq(&$a, $skipped = null) {
        ...
        $this->logger->debug("excel row: ".json_encode($a, JSON_UNESCAPED_UNICODE));
        //map to table record
        $record = $this->mapRecord($a, $this->instant_table_mappings);
        $this->logger->debug("validateRecord: ".json_encode($record, JSON_UNESCAPED_UNICODE));
        //check account
        $record['account'] = preg_replace('/\D/', '', $record['account']);  //remove non-digit
        if (preg_match('/^\d+$/', $record['account'])==false)
            return ['code'=>101, 'msg'=>'Beneficiary Account No. invalid'];
...
*/
    ...
    private function mapRecord($a, $mappings = null) {
        ...
        $this->logger->debug("mapRecord - a: " . json_encode($a, JSON_UNESCAPED_UNICODE));
        $this->logger->debug("mapRecord - mapping: ".json_encode($mappings, JSON_UNESCAPED_UNICODE));
...
*/
    ...
    public function getMerchantFeeConfig($mid, $target) {
        ...
        $charge_rate = (is_null($r['remittance_fee'])?self::DEFAULT_REMITTANCE_CHARGE_RATE:$r['remittance_fee']);
...
*/
    ...
    public function writeExcelMerchantReport1($batchs, $final=true)  {
        // charge = 0.5%
        $charge_rate = (is_null($batchs[0]['remittance_fee'])?self::DEFAULT_REMITTANCE_CHARGE_RATE:$batchs[0]['remittance_fee']);
        $charge_rate.='%';
        // service minimum charge in USD
 ...
 */
    ...
    public function getUsdChargeFee($batch, $usd) {
        // in %
        $charge_rate = (is_null($batch['remittance_fee'])?self::DEFAULT_REMITTANCE_CHARGE_RATE:$batch['remittance_fee']);
        // service minimum charge
