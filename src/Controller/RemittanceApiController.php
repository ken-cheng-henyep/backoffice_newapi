...
*/
    ...
    public function instantRequest($skipped_fields=null) {
        ...
        $reqs = $this->request->data;
        $this->log("before process", 'debug');
        $this->log("request: ".json_encode($reqs, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES), 'debug');

        //disabled for all - mod by Ken Cheng 2019-07-29
        ...
        } */ // Commented since it block all merchants action for testing
        ...
        $this->log("balance: ".($balance_ok?"true":"false")."\npaid_amount: ($paid_amount!=false?$paid_amount:'false')", 'debug');
        //skip for test_trans
        if (!$test_trans)
